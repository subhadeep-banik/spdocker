...
static const uint8_t sbox_bsf6[256] =   {
...
static void rotatef6(unsigned char *in) {
    unsigned char a,c;
    a = in[0];
    for(c=0;c<3;c++) 
        in[c] = in[c + 1];
...
static unsigned char rconf6(unsigned char in) {
    unsigned char c=1;
    if(in == 0)  
        return 0; 
    while(in != 1) {
...
static void schedule_coref6(unsigned char *in, unsigned char i) {
    char a;
...
static void schedule_subf6(unsigned char *in ) {
    char a;
    /* Rotate the input 8 bits to the left */
