...
void bs_transposef6(word_t * blocks)
{
    ...
    bs_transpose_dstf6(transpose,blocks);
...
void bs_expand_keyf6(word_t (* rk)[BLOCK_SIZE], uint8_t * _key)
{
    // TODO integrate this better
    uint8_t key[KEY_SCHEDULE_SIZE];
    memmove(key,_key,BLOCK_SIZE/4);
    expand_keyf6(key);
...
{
    int round;
    bs_transposef6(state);


    bs_addroundkeyf6(state,rk[0]);
    for (round = 1; round < 14; round++)
    {
        bs_apply_sboxf6(state);
        /*bs_shiftrows(state);*/
        /*bs_mixcolumns(state);*/
        bs_shiftmixf6(state);
        bs_addroundkeyf6(state,rk[round]);
    }
    bs_apply_sboxf6(state);
    bs_shiftrowsf6(state);
    bs_addroundkeyf6(state,rk[14]);
    bs_transpose_revf6(state);
}
...
void bs_cipher_revf6(word_t state[BLOCK_SIZE], word_t (* rk)[BLOCK_SIZE])
{
    ...
    bs_addroundkeyf6(state,rk[10]);
    for (round = 13; round > 0; round--)
    {
        bs_shiftrows_revf6(state);
        bs_apply_sbox_revf6(state);
        bs_addroundkeyf6(state,rk[round]);
        bs_mixcolumns_revf6(state);
    }
    bs_shiftrows_revf6(state);
    bs_apply_sbox_revf6(state);
    bs_addroundkeyf6(state,rk[0]);
